<template>
  <div class="container thoridal-list">
    <div class="light-block list-header action-row-container">
      ...
      <div class="action-row">
        ...
        <el-dropdown @command="handleHeadDropdown" trigger="click">
          ...
          <template #dropdown>
            <el-dropdown-menu class="th-header-dropdown" tabindex="-1">
              <el-dropdown-item command="modify-password"
                ><span class="iconfont icon-edit"></span> Change
                Password</el-dropdown-item
              >
              <el-dropdown-item command="send-backup"
                ><span class="iconfont icon-modify"></span> Send
                Backup</el-dropdown-item
              >
              <el-dropdown-item command="create-user" divided v-if="isAdmin"
                ><span class="iconfont icon-collection"></span> Create
    ...
    <div class="list-content">
      ...
      >
        <div class="action-row-container">
          ...
          <div class="action-row">
            ...
            <el-button
              type="text"
              icon="el-icon-delete"
              @click="deleteRecord(item, itemIndex)"
              tabindex="-1"
            ></el-button>
          </div>
        </div>
        <div class="info-container">
          <p
            @click="$root.copyText(item.Data.Account)"
            v-if="item.Data.Account"
          >
            <span class="iconfont icon-account"></span>{{ item.Data.Account }}
          </p>
          <p
            @click="$root.copyText(item.Data.Password)"
            v-if="item.Data.Password"
          >
            <span class="iconfont icon-password"></span
            ><span
              class="iconfont pwd-mask"
    ...
    <div
      class="loadmore"
      v-show="listCtr.page < listCtr.total"
      @click="fetchRecordList(listCtr.page + 1)"
      :title="listCtr.page + '/' + listCtr.total"
    >
    ...
    <div
      class="empyt-placeholder"
      v-show="listData.length === 0 && !listCtr.isRequesting"
    >
...
import recordEditorView from "../components/record-editor.vue";
import userCreatorView from "../components/user-creator.vue";
