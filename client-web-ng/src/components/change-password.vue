...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      const oldToken = await genUserToken(
        this.auth.name,
      ...
      const isConfirm = await this.$root.confirm(
        "Please keep your new password properly. Continue ?",
        "Warning"
      );
      if (!isConfirm) {
        return;
      ...
      try {
        ...
        const payload = {};
        payload.Token = await genUserToken(
          this.auth.name,
          this.form.newPassword
        );
        const { data, iv } = await cipher.encryptText(
          this.auth._decryptedKey,
          this.form.newPassword,
          true
        );
        payload.Key = {
